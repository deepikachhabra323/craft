...
export async function searchWorkerWithout(eid){
    // createIndex();
    await connect();
    const repository = client.fetchRepository(mappingSchema);
    // console.log(eid,'eid')
    let mappings =  await repository.search().where('contracts').does.not.match(eid).returnAll();
    // console.log('mapping',mappings)
    return mappings
}
...
export async function updateWorker(eid,cid,allocation){
    await connect();

    const repository = client.fetchRepository(mappingSchema);

    const workers = await repository.search().where('emp').eq(eid).returnAll();
    let worker = workers[0];
    let contracts = worker.contracts=='offboard'?[]:JSON.parse(worker.contracts);
    contracts.push(cid);
    let allocationW = JSON.parse(worker.allocation);
    allocationW.push(allocation)
    // worker.emp = worker.emp;
    // console.log('worker',worker.contracts,worker.allocation,)
    worker.contracts = JSON.stringify(contracts);
    worker.allocation = JSON.stringify(allocationW);
